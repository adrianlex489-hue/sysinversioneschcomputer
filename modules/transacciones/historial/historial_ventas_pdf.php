<?php
// ============================================================
// modules/transacciones/historial/historial_ventas_pdf.php
// Genera PDF A4 del Historial de Ventas con FPDF
...
$wheres = []; $params = [];
if ($filtro_est  !== 'all') { $wheres[] = 'v.estado = ?';              $params[] = $filtro_est; }
if ($filtro_pago !== 'all') { $wheres[] = 'v.tipo_pago = ?';           $params[] = $filtro_pago; }
if ($fecha_desde !== '')    { $wheres[] = 'DATE(v.fecha) >= ?';        $params[] = $fecha_desde; }
if ($fecha_hasta !== '')    { $wheres[] = 'DATE(v.fecha) <= ?';        $params[] = $fecha_hasta; }
...
// ── Clase PDF ─────────────────────────────────────────────────────────────────
class HistorialVentasPDF extends FPDF
{
    ...
    private array $cVerdeMedio = [ 39, 174,  96];
    ...
    private array $cVerdePale  = [232, 245, 233];
    ...
    public function Header(): void
    {
        ...
        $this->SetXY(249, 5);
        $this->Cell(42, 6, 'VENTAS', 0, 1, 'C');
        $this->SetFont('Arial', '', 7);
        $this->SetXY(249, 11);
        $this->Cell(42, 5, 'HISTORIAL', 0, 0, 'C');

        // Banda subtítulo
        $this->SetFillColor(...$this->cVerdeMedio);
        $this->Rect(0, 22, 297, 10, 'F');
        $this->SetTextColor(...$this->cBlanco);
        $this->SetFont('Arial', 'B', 8.5);
        $this->SetXY(6, 23);
        $this->Cell(160, 8, pdf_txt('Historial de Ventas — ' . date('d/m/Y H:i') . '   |   Estado: ' . $this->label_estado), 0, 0, 'L');
        $this->SetFont('Arial', '', 7.5);
        $this->SetXY(166, 23);
    ...
    public function Footer(): void
    {
        ...
        $this->Cell(0, 4, pdf_txt('SysInversiones CH Computer  |  Historial de Ventas  |  Pagina ' . $this->PageNo() . '  |  Generado el ' . date('d/m/Y H:i:s')), 0, 0, 'C');
    }

    // Anchos: #, Fecha, Cliente, Comprobante, N°, T.Pago, Método, Total, Saldo, Estado, Vendedor
    // Total: 10+28+52+22+26+16+16+22+22+18+30 = 262 mm (A4 landscape útil ~277)
    ...
    public function tablaFila(array $r, int $n): void
    {
        $cols  = $this->getCols();
        $esAlt = ($n % 2 === 0);
        $fillBg = $esAlt ? $this->cVerdePale : $this->cBlanco;
        ...
        $valores = [
            '#' . $r['id_venta'],
            $r['fecha'],
            mb_substr($r['nombre_cliente'] ?? '', 0, 32),
    ...
    public function filaResumen(): void
    {
        ...
        $resumen = '  TOTAL: ' . $this->total_reg . ' ventas   |   Pagadas: ' . $this->total_pag . '   |   Pendientes: ' . $this->total_pend . '   |   Anuladas: ' . $this->total_anul . '   |   Monto total: S/. ' . number_format($this->total_monto, 2) . '   |   Generado el ' . date('d/m/Y H:i:s');
        $this->Cell(0, 7, pdf_txt($resumen), 0, 1, 'L', true);
        $this->SetTextColor(...$this->cTexto);
    }
}

// ── Instanciar y generar PDF — A4 Landscape ───────────────────────────────────
$pdf = new HistorialVentasPDF('L', 'mm', 'A4');
$pdf->empresa      = $empresa;
...
$pdf->SetTitle('Historial de Ventas - SysInversiones CH Computer');
...
if (empty($rows)) {
    ...
    $pdf->Cell(0, 20, pdf_txt('No se encontraron ventas con los filtros seleccionados.'), 0, 1, 'C');
} else {
    $pdf->tablaCabecera();
    $n = 1;
    foreach ($rows as $r) {
...
ob_end_clean();
$filename = 'historial_ventas_' . date('Ymd_His') . '.pdf';
